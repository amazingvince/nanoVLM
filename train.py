...
def train(
    ...
    if is_master():
        ...
        print(
            f"Validation summary{' (global)' if is_dist() else ''}: {-1 * get_world_size()} samples, {int(len(val_loader) * get_world_size())} batches/epoch, batch size {int(train_cfg.batch_size * get_world_size() * train_cfg.gradient_accumulation_steps)}{', training on ' + str(get_world_size()) + ' GPUs' if is_dist() else ''}"
        )
        if is_dist():
            print(
                                ...
                                try:
                                    ...
                                    if step not in logged_eval_steps:
                                        ...
                                        if lmms_results:
                                            ...
                                            )  # We need to the global step otherwise wandb raises the step counter
...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--vit_img_size",
        type=int,
        help="Size of EACH patch sent to vision encoder (default: 512). This is the resolution the vision encoder processes. DINOv3's 2D RoPE enables training at higher resolutions than its 224×224 pretraining.",
    )
    parser.add_argument(
        "--max_img_size",
        type=int,
        help="Maximum size of WHOLE image before splitting into vit_img_size patches (default: 1024). Images larger than this are resized down, then split into patches of vit_img_size.",
    )
