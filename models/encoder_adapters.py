...
from models.grid_abstraction import DINOv3Grid, GridFactory, ImageGrid, SigLIPGrid
...
class VisionEncoderAdapter(ABC):
    ...
    @abstractmethod
    def prepare_images(
        self, images: List[torch.Tensor], grids: List[Any]
    ) -> Tuple[torch.Tensor, List[ImageGrid]]:
        """
...
class SigLIPAdapter(VisionEncoderAdapter):
    ...
    def prepare_images(
        ...
        for grid_raw in flattened_grids:
            ...
            grid = SigLIPGrid(
                rows=n_h, cols=n_w, tokens_per_tile=self.config.mp_image_token_length
            )
        ...
        batched_tiles = (
            torch.cat(all_tiles, dim=0) if all_tiles else torch.empty(0, 3, 224, 224)
        )
        ...
        for grid_raw in grids:
            ...
            if not isinstance(grid, DINOv3Grid):
                ...
                if isinstance(grid_raw, dict) and "HpWp" in grid_raw:
                    grid = DINOv3Grid.from_raw(
                        grid_raw, self.config.mp_pixel_shuffle_factor
                    )
                else:
                    # Fallback
    ...
    def process_embeddings(
        ...
        for i, grid in enumerate(grids):
            ...
            Hp, Wp = grid.get_modality_projector_dims(
                self.config.mp_pixel_shuffle_factor
            )
