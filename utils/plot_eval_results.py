...
def compute_ranking_summary(all_results, tasks_to_plot):
    ...
    for step in all_steps:
        ...
        if tasks_to_plot:
            ...
            metrics_to_rank = [
                k
                for k in step_data[0].keys()
                if k not in ["step", "average", "average_rank"]
            ]
...
def load_eval_results(eval_folder, tasks_to_plot=None):
    ...
    for json_file in json_files:
        with open(json_file, "r") as f:
            ...
            if tasks_to_plot and any(
                "mme_total_score" in task.lower() for task in tasks_to_plot
            ):
            ...
            if tasks_to_plot and "average" in tasks_to_plot:
                ...
                if metrics_to_average:
                    result["average"] = sum(metrics_to_average) / len(
                        metrics_to_average
                    )
...
def plot_results(
    ...
    for i, metric in enumerate(metric_names):
        ...
        for results in all_results:
            for result in results:
                if (
                    steps_to_plot is None or result["step"] in steps_to_plot
                ) and metric in result:
...
def main():
    eval_folders, custom_names, tasks_to_plot, output_filename, steps_to_plot = (
        parse_args()
    )
    ...
    for eval_folder in eval_folders:
        ...
        if results:
            ...
            if steps_to_plot:
                ...
                missing_steps = [
                    step for step in steps_to_plot if step not in available_steps
                ]
            ...
            if tasks_to_plot:
                ...
                missing_tasks = []
                for task in tasks_to_plot:
                    if task not in available_metrics and task not in [
                        "average",
                        "average_rank",
                    ]:
